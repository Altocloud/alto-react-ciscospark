...
export function fetchFreeGuest(account) {
  // in development on localhost, we should just use hard coded tokens
  console.log('COLM: ', account);
  if (process.env.NODE_ENV === `development`) {
    return {
      then: (callback) => {
        callback({
          accessToken: `YmY1ZjM0YTUtMDY5MC00Y2ZjLWEyYjItMDY0M2U2MjAzYTZhMmI4NDUzYzEtNTIx`,
          refreshToken: ``,
          sparkId: `123dev456`
        });
        return {
...
export function setGuestAsBusy(account, sparkId, interaction) {
  // in development on localhost, we should just use hard coded tokens
  if (process.env.NODE_ENV === `development`) {
    return {
      then: (callback) => {
