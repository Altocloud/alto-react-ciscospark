...
class SparkComponent extends Component {
  ...
  componentDidMount() {
    ...
    if (!spark) {
      ...
      spark = createSpark(accessToken, refreshToken);
      metrics.sendLoadStart(spark);
      metrics.sendMetricsInQueue(spark);
      metrics.sendElapsedTimeMetric(`sparkObject:loaded`, spark);
      props.storeSparkInstance(spark);
    }

    spark.listenToAndRun(spark, `change:canAuthorize`, () => {
      metrics.sendElapsedTimeMetric(`sparkObject:authenticated`, spark);
    ...
    this.connectToMercuryOnce = _.once(props.connectToMercury);
  ...
  setupMercury(spark, props) {
    ...
    else if (registered && authenticated && !connected && !connecting && !hasConnected) {
      this.connectToMercuryOnce(spark);
    }
  }
