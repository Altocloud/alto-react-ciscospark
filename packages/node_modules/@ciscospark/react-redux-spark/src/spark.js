/* eslint camelcase: [0] */

import '@ciscospark/plugin-logger';
import '@ciscospark/plugin-avatar';
import '@ciscospark/plugin-conversation';
import '@ciscospark/plugin-phone';
import '@ciscospark/plugin-metrics';
import '@ciscospark/plugin-flag';
import Spark from '@ciscospark/spark-core';
import LocalStorageStoreAdapter from '@ciscospark/storage-adapter-local-storage';

// (colm.seale) edited ciscospark code to allow refresh token to be passed
export default function createSpark(accessToken, refreshToken) {
  const credentials = JSON.parse(localStorage.getItem(`credentials`) || false);
  return new Spark({
    credentials: {
      authorization: {
        access_token: accessToken || credentials.access_token,
        refresh_token: refreshToken || credentials.refresh_token
      }
    },
    config: {
      logger: {
        level: process.env.NODE_ENV === `production` ? undefined : `trace`
      },
      conversation: {
        allowedInboundTags: {
          'spark-mention': [`data-object-type`, `data-object-id`, `data-object-url`],
          a: [`href`],
          b: [],
          blockquote: [`class`],
          strong: [],
          i: [],
          em: [],
          pre: [],
          code: [],
          br: [],
          hr: [],
          p: [],
          ul: [],
          ol: [],
          li: [],
          h1: [],
          h2: [],
          h3: []
        },
        allowedOutboundTags: {
          'spark-mention': [`data-object-type`, `data-object-id`, `data-object-url`],
          a: [`href`],
          b: [],
          blockquote: [`class`],
          strong: [],
          i: [],
          em: [],
          pre: [],
          code: [],
          br: [],
          hr: [],
          p: [],
          ul: [],
          ol: [],
          li: [],
          h1: [],
          h2: [],
          h3: []
        }
      },
      storage: {
        boundedAdapter: new LocalStorageStoreAdapter(`ciscospark-embedded`)
      }
    }
  });
}
