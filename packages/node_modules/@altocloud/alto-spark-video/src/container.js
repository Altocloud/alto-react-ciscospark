...
class AltoSparkVideoComponent extends Component {
  ...
  getFreeGuestAccount(account) {
    ...
    sparkGuest.getFreeUser().then(({sparkId, accessToken, refreshToken}) => {
      sparkGuest.setUserAsBusy(account.id, sparkId, this.props.interaction).then(() => {
        this.setState({
          accessToken,
          refreshToken
        });
  ...
  render() {
    if (!this.state.getFreeUser && this.props.interaction && this.props.interaction.id && this.props.account) {
      this.getFreeUser = true;
      this.getFreeGuestAccount(this.props.account);
    }
