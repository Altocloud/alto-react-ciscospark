    ...
    if (spark && connected && authenticated && registered) {
      ...
      if (!user.get(`currentUser`) && !user.getIn([`status`, `isFetchingCurrentUser`])) {
        nextProps.fetchCurrentUser(spark, (data) => {
          openInteraction(interaction).then(() => updateInteraction(interaction.id, {
            provider: {
              type: `spark`,
              sid: data.id
            }
      ...
      // handle closing interaction when call is finished
      ...
      if (user.get(`toUser`) && !isActiveCall && !isIncomingCall) {
